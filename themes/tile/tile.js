...
function image_show(i) {
	if (image_width[i] < 0) {
		...
		ui_sethtml('image_view', '<iframe style="width:100%;height:' + (div.offsetHeight - 60) + 'px" frameborder="0" src="' + document.getElementById('img_' + i).href + '"></iframe>');
	} else {
		ui_sethtml('image_view', '<img src="' + document.getElementById('img_' + i).href + '" width="' + image_width[i] + '" height="' + image_height[i] + '" onload="image_loaded()" alt=""/>');
		ui_vis('image', 1);
	}
