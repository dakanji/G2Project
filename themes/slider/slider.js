...
function image_show(i) {
	...
	if (data_iw[i] < 0) {
		image_div.innerHTML = '<iframe style="width:100%;height:' + (image_area.offsetHeight - 4) + 'px" frameborder="0" src="' + document.getElementById('img_' + i).href + '"></iframe>';
		ui_vis('fit_size', 0); ui_vis('full_size', 0);
	} else {
		var s               = image_fit(1);
		image_div.innerHTML = '<img name="slide" src="' + document.getElementById('img_' + i).href + '" ' + s + ' onload="image_loaded()" alt=""' + image_map + '/>';
	}
	image_setbuttons();
...
function options_setsize() {
	...
	if (app_is_ie) {
		document.getElementById('options').style.height = (image_area.offsetHeight - 4) + 'px';
	}
}
