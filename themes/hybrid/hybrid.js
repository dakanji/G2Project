...
function app_onresize() {
	...
	if (app_is_ie) {
		document.getElementById('popup_details').firstChild.style.height = Math.round(app_wh * 0.75) + 'px';
	}
}
function app_onload() {
	if (data_count > 0) {
...
function app_setcookie() {
	var d = new Date(), c = slide_order + ';' + (slide_delay / 1000) + ';' + sidebar_on + ';' + album_detailson + ';' + album_itemlinkson + ';' + text_on + ';';
...
function album_setmargin() {
	document.getElementById('gsAlbumContent').style.marginTop = document.getElementById('album_titlebar').offsetHeight + 'px';
}
...
function image_show(i) {
	...
	image_setsize();
	if (data_iw[i] < 0) {
		imagediv.innerHTML = '<iframe style="width:100%;height:' + (imagearea.offsetHeight - 4) + 'px" frameborder="0" src="' + document.getElementById('img_' + i).href + '"></iframe>';
		ui_vis('fit_size', 0); ui_vis('full_size', 0);
	} else {
		var s              = image_fit(1);
		imagediv.innerHTML = '<img name="view" src="' + document.getElementById('img_' + i).href + '" ' + s + ' onload="image_loaded()" alt=""/>';
	}
	image_setbuttons();
	text_fill();
...
function popup_menu(event,i,ii) {
	...
	var links            = ii >= 0 ? '<a href="" onclick="popup_info(' + ii + ');this.blur();return false">' + item_details + '</a><br/>' : '';
	if (obj) {
		links += obj.innerHTML;
	}
	pop.style.left  = '0';
