...
function highlightCurrentThumb(thumbIndex) {
	...
	thumbnails[currentImageIndex].className = thumbnails[currentImageIndex].className.replace(/\bcurrentImage\b/, '');
...
function downSizePhoto() {
	if (parseInt(mainPhoto.style.width) > PHOTO_MINIMISED
		...
		if (parseInt(getElementStyle(mainPhoto.id, 'left')) > 40) {
			mainPhoto.style.left = (parseInt(getElementStyle(mainPhoto.id, 'left')) - mainPhoto.origLeft / 12) + 'px';
		}
...
function upSizePhoto() {
	if (parseInt(mainPhoto.style.width) < mainPhoto.origWidth
		...
		if (parseInt(getElementStyle(mainPhoto.id, 'left')) < mainPhoto.origLeft) {
			mainPhoto.style.left = (parseInt(getElementStyle(mainPhoto.id, 'left')) + mainPhoto.origLeft / 10) + 'px';
		}
...
function insertimagedatablock() {
	...
	fullsizeLink.href = thumbnails[currentImageIndex].parentNode.href + (thumbnails[currentImageIndex].parentNode.href.indexOf('?') >= 0 ? '&' : '?') + 'thumbIndex=' + currentImageIndex;
