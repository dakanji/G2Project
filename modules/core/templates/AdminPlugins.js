...
function updateStateCounts() {
	...
	for (i in states) {
		...
		if (!counts[state]) {
			...
			msgTopEl.innerHTML     = msgBottomEl.innerHTML = legendStrings[state].replace('__COUNT__', counts[state]);
		}
	}
...
function addMessage(pluginType, pluginId, messageText, messageType) {
	...
	containerEl.style.left = (((bodyDimensions.right - bodyDimensions.left) - (statusDimensions.right - statusDimensions.left)) / 2) + "px";
