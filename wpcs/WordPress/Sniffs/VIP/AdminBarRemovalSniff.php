...
class AdminBarRemovalSniff extends AbstractFunctionParameterSniff {
	...
	public function process_token($stackPtr) {
		...
		$file_extension = mb_substr(mb_strrchr($file_name, '.'), 1);
	...
	public function process_text_for_style($stackPtr, $file_name) {
		...
		if (true === $this->in_style[ $file_name ]) {
			if (false !== mb_strpos($content, '</style>')) {
				// Make sure we check any content on this line before the closing style tag.
				$this->in_style[ $file_name ] = false;
				$content                      = trim(mb_substr($content, 0, mb_strpos($content, '</style>')));
			}
			...
			if (false === mb_strpos($content, '</style>')) {
				...
				$content                      = trim(mb_substr($content, (mb_strpos($content, '<style') + 6)));
			} else {
				// Ok, we have open and close style tag on the same line with possibly content within.
				$start   = (mb_strpos($content, '<style') + 6);
				$end     = mb_strpos($content, '</style>');
				$content = trim(mb_substr($content, $start, ($end - $start)));
				unset($start, $end);
			}
		} else {
			return;
		...
		if (true === $this->in_target_selector[ $file_name ]) {
			if (false !== mb_strpos($content, '}')) {
				...
				$this->in_target_selector[ $file_name ] = false;
				$content                                = trim(mb_substr($content, 0, mb_strpos($content, '}')));
			}
			...
			if (isset($matches[1]) && '' !== $matches[1]) {
				if (false === mb_strpos($matches[1], '}')) {
					...
					$content = trim(mb_substr($matches[1], 0, mb_strpos($matches[1], '}')));
				}
			} else {
				$this->in_target_selector[ $file_name ] = true;
			}
		...
		if (! empty($content)) {
			foreach ($this->target_css_properties as $property => $requirements) {
				if (false !== mb_strpos($content, $property)) {
					$error = true;
	...
	protected function process_css_style($stackPtr) {
		...
		if (false !== $opener) {
			...
			foreach ($this->target_css_selectors as $target_selector) {
				if (false !== mb_strpos($selector, $target_selector)) {
					$error = true;
