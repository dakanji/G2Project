...
class FileNameSniff extends Sniff {
	...
	public function process_token($stackPtr) {
		...
		$expected = mb_strtolower(str_replace('_', '-', $fileName));
		...
		if (true === $this->strict_class_file_names) {
			...
			if (false !== $has_class && false === $this->is_test_class($has_class)) {
				...
				$expected   = 'class-' . mb_strtolower(str_replace('_', '-', $class_name));

				if (mb_substr($fileName, 0, -4) !== $expected && ! isset($this->class_exceptions[ $fileName ])) {
					$this->phpcsFile->addError(
		...
		if (false !== mb_strpos($file, DIRECTORY_SEPARATOR . 'wp-includes' . DIRECTORY_SEPARATOR)) {
			...
			if (false !== $subpackage_tag) {
				...
				if (false !== $subpackage) {
					$fileName_end = mb_substr($fileName, -13);
					$has_class    = $this->phpcsFile->findNext(T_CLASS, $stackPtr);
					...
					if (('Template' === trim($this->tokens[ $subpackage ]['content'])
						...
						$this->phpcsFile->addError(
							...
							array(
								mb_substr($fileName, 0, -4) . '-template.php',
								$fileName,
							)
						);
					}
