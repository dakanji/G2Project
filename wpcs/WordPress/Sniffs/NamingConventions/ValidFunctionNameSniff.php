...
class ValidFunctionNameSniff extends PHPCS_PEAR_ValidFunctionNameSniff {
	...
	protected function processTokenOutsideScope(File $phpcsFile, $stackPtr) {
		...
		if (0 === mb_strpos($functionName, '__')) {
		...
		if (mb_strtolower($functionName) !== $functionName) {
	...
	protected function processTokenWithinScope(File $phpcsFile, $stackPtr, $currScope) {
		...
		if (mb_strtolower($methodName) !== $methodName) {
	...
	protected function get_name_suggestion($name) {
		...
		$suggested = mb_strtolower($suggested);
