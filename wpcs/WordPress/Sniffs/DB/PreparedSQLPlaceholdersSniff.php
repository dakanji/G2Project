...
class PreparedSQLPlaceholdersSniff extends Sniff {
		...
		for ($i = $query['start']; $i <= $query['end']; $i++) {
			...
			if (! isset(Tokens::$textStringTokens[ $this->tokens[ $i ]['code'] ])) {
				...
				if (T_STRING === $this->tokens[ $i ]['code']) {
					if ('sprintf' === mb_strtolower($this->tokens[ $i ]['content'])) {
					...
					} elseif ('implode' === mb_strtolower($this->tokens[ $i ]['content'])) {
				...
				if (! empty($walk)) {
					foreach ($walk as $match_key) {
						foreach ($matches[ $match_key ] as $index => $match) {
							...
							if (mb_strpos($match, '%') === false && mb_strpos($match, '_') === false) {
	...
	protected function analyse_sprintf($sprintf_params) {
		...
		foreach ($sprintf_params as $sprintf_param) {
			if (mb_strpos(mb_strtolower($sprintf_param['raw']), 'implode') === false) {
				continue;
			}
			...
			if (T_STRING === $this->tokens[ $implode ]['code']
				&& 'implode' === mb_strtolower($this->tokens[ $implode ]['content'])
			) {
	...
	protected function analyse_implode($implode_token) {
		...
		if (mb_strpos(mb_strtolower($implode_params[2]['raw']), 'array_fill') === false) {
			return false;
		}
		...
		if (T_STRING !== $this->tokens[ $array_fill ]['code']
			|| 'array_fill' !== mb_strtolower($this->tokens[ $array_fill ]['content'])
		) {
			return false;
		}
