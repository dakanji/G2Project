...
abstract class AbstractSniffUnitTest extends PHPUnit_Framework_TestCase {
	...
	protected function getTestFiles($testFileBase) {
		...
		$dir = mb_substr($testFileBase, 0, mb_strrpos($testFileBase, DIRECTORY_SEPARATOR));
		$di  = new DirectoryIterator($dir);

		foreach ($di as $file) {
			$path = $file->getPathname();

			if (mb_substr($path, 0, mb_strlen($testFileBase)) === $testFileBase) {
				// Start of WPCS adjustment
				// If we're changing things anyway, we may as well exclude backup files
				// from the test runs ;-)
				if ($path !== $testFileBase . 'php' && mb_substr($path, -5) !== 'fixed'
					&& mb_substr($path, -3) !== 'bak' && mb_substr($path, -4) !== 'orig'
				) {
					$testFiles[] = $path;
				}
				// End of WPCS adjustment
	...
	final public function testSniff() {
		...
		$basename = mb_substr(get_class($this), 0, -8);
		...
		$standardName = mb_substr($basename, 0, mb_strpos($basename, '_'));
