...
if (!defined('_ADODB_LAYER')) {
	...
	global
		...
		$ADODB_GETONE_EOF,
		// Allows you to force quotes (backticks) around field names in queries generated by getinsertsql and getupdatesql.
	...
	abstract class ADOConnection {
		...
		public $hasMoveFirst = false;

		/// can generate sequences using GenID();
		public $hasGenID = false;
		...
		public $_errorCode = false;

		/// This variable keeps the last created result link identifier
		public $_queryID = false;

		/// A boolean variable to state whether its a persistent connection or normal connection.	*/
		public $_isPersistentConnection = false;
		...
		*/
		...
		public function CompleteTrans($autoComplete = true) {
			...
			if ($this->_transOK && $autoComplete) {
				if (!$this->CommitTrans()) {
					...
					if ($this->debug) {
						self::outp('Smart Commit failed');
					}
				} else {
					if ($this->debug) {
						self::outp('Smart Commit occurred');
					}
				}
			} else {
				$this->_transOK = false;
				...
				if ($this->debug) {
					self::outp('Smart Rollback occurred');
				}
			}
		...
		public function GenID($seqname = 'adodbseq', $startID = 1) {
			if (!$this->hasGenID) {
				// formerly returns false pre 1.60
				return 0;
			}
		...
		public function Affected_Rows() {
			...
			if ($this->debug) {
				self::outp('<p>Affected_Rows error</p>', false);
			}
		...
		public function CacheExecute($secs2cache, $sql = false, $inputarr = false) {
			...
			if (!$rs) {
				...
				if ($this->debug) {
					...
					if ($this->debug !== -1) {
						self::outp(" $md5file cache failure: $err (this is a notice and not an error)");
					}
				}
		...
		*/
		...
		public function BeginTrans() {
			if ($this->debug) {
				self::outp('BeginTrans: Transactions not supported for this driver');
			}
	...
	class ADORecordSet implements IteratorAggregate {
		...
		public function GetAssoc($force_array = false, $first2cols = false) {
			...
			// workaround accelerator incompat with PHP 4.4 :(
			$ref = $results;
	...
	class ADORecordSet_array extends ADORecordSet {
		...
		public function Fields($colname) {
			...
			if ($mode & ADODB_FETCH_ASSOC) {
				if (!isset($this->fields[$colname]) && null !== $this->fields[$colname]) {
