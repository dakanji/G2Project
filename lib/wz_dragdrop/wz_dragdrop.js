...
function WZDD()
{
	...
	this.db       = (document.compatMode && document.compatMode.toLowerCase() != "backcompat") ? document.documentElement
		: (document.body || null);
...
dd.initz = function () {
	...
	if (dd.op6) {
		WINSZ(2);
	} else if (dd.n6 || dd.ie || dd.op && !dd.op6 || dd.w3c) {
		dd.recalc(1);
	}
	...
	if (d_drag && document.onmousemove != DRAG) {
		dd.setEvtHdl(1, DRAG);
	} else if (d_resize && document.onmousemove != RESIZE) {
		dd.setEvtHdl(1, RESIZE);
	}
	if ((d_drag || d_resize) && document.onmouseup != DROP) {
		dd.setEvtHdl(2, DROP);
	}
	dd.setEvtHdl(0, PICK);
};
...
DDObj.prototype.copy = function (d_n, d_p) {
	...
	while (d_n--) {
		...
		if (dd.n4) {
			...
			d_p.write(d_o.htm);
			d_p.close();
		} else if (dd.db.insertAdjacentHTML) {
			dd.db.insertAdjacentHTML("AfterBegin", d_o.t_htm);
		...
		} else if (dd.db && dd.db.innerHTML) {
			dd.db.innerHTML += d_o.t_htm;
		}
		...
		if (dd.obj.nodrag) {
			...
			if (d_rsz) {
				...
				if (dd.ie) {
					...
					setTimeout(
						'if (dd.obj && document.selection && document.selection.empty)' + '{' + 'document.selection.empty();' + 'if (dd.ie4) window.scrollTo(window.dd_x, window.dd_y);' + '}'
						,
						0
					);
				}
...
function DROP(d_ev)
{
	if (dd.obj) {
		...
		if (!dd.n4 && !dd.op6 || !dd.obj.is_image) {
			dd.recalc();
		}
