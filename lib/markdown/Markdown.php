...
class Markdown implements MarkdownInterface {
	...
	public function __construct() {
		...
		$tempVarA                 = str_repeat('(?>[^\[\]]+|\[', $this->nested_brackets_depth);
		$tempVarB                 = str_repeat('\])*', $this->nested_brackets_depth);
		$this->nested_brackets_re = $tempVarA . $tempVarB;

		// nested_url_parenthesis_depth
		$tempVarA                        = str_repeat('(?>[^()\s]+|\(', $this->nested_url_parenthesis_depth);
		$tempVarB                        = str_repeat('(?>\)))*', $this->nested_url_parenthesis_depth);
		$this->nested_url_parenthesis_re = $tempVarA . $tempVarB;
	...
	protected function _doImages_reference_callback($matches) {
		...
		if (isset($this->urls[$link_id])) {
			...
			// If there is no such link ID, leave intact:
			$result = $whole_match;
		}
