...
class MarkdownExtra extends \Michelf\Markdown {
	...
	/**
	 * Only convert atx-style headers if there is a space between the header and #
	 * @var boolean
	 */
	 ...
	 * @var string
	 */
	protected $block_tags_re = 'p|div|h[1-6]|blockquote|pre|table|dl|ol|ul|address|form|fieldset|'
	. 'iframe|hr|legend|article|section|nav|aside|hgroup|header|footer|figcaption|figure';
	...
	protected function _doHeaders_callback_atx($matches) {
		...
		if (is_callable($this->header_id_func)) {
			$defaultId = call_user_func($this->header_id_func, $matches[2]);
		} else {
			$defaultId = null;
		}
	...
	protected function appendFootnotes($text) {
		...
		if (!empty($this->footnotes_ordered)) {
			...
			while (!empty($this->footnotes_ordered)) {
				...
				unset($this->footnotes_ref_count[$note_id], $this->footnotes[$note_id]);
