...
function init() {
	...
	if (isIE) {
		...
		var strNewHTML = "<span id='" + imgID + "'" + imgTitle + " style=\"width:192px; height:192px;display:inline-block;" + "filter:progid:DXImageTransform.Microsoft.AlphaImageLoader" + "(src=\'" + img.src + "\', sizingMethod='scale');\"></span>";
...
function hueUpdate() {
	...
	document.getElementById("Markup_pickerDiv").style.backgroundColor = "rgb(" + rgb.join(",") + ")";
...
function swatchUpdate() {
	...
	document.getElementById("Markup_swatch").style.backgroundColor = "rgb(" + rgb.join(",") + ")";
	...
	document.getElementById("Markup_hexval").value = YAHOO.util.Color.rgb2hex(rgb[0], rgb[1], rgb[2]);
}
