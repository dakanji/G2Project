...
function bsw_BlockPreference(key, blockId, values) {
	...
	this.toString = function () {
		...
		for (var paramName in this.parameters) {
			...
			var defaultValue = bsw_WIDGET_BLOCKS[key][blockId]['parameters'][paramName]['defaultValue'];
	...
	/* Override whatever we are specifically changing */
...
function bsw_reInitAdminForm(key) {
	/*
	 * Do not reinitialize if we are already inside an update.  This can happen
...
function bsw_showBlockOptions(key) {
	...
	/* For some reason, "block.parameters.length == 0" does not work here.  wtf? */
	var i = 0;
...
function bsw_getValueElement(paramName, blockPref, block) {
	...
	if (elementType == 'boolean') {
		...
		* it is because the box was checked via the UI.
		*/
		...
		for (var choice in block.parameters[paramName]['extra']) {
			...
			if (choice == prefValue) {
				callback = new Function('document.getElementById("' + id + '").selectedIndex = ' + i + ';');
			}
			i++;
		}
		...
		callback               = new Function('document.getElementById("' + id + '").value = "' + prefValue.replace(/\"/g, '\\"') + '";');
...
function bsw_updateAlbumBlockValue(key) {
	...
	/*
	 * Do not change it unless we really need to, to avoid unnecessarily
